...
export default function AddNewBookPage() {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<BookFormInputs>({
    defaultValues: {
      totalCopies: 1,
      availableCopies: 1,
    }
  ...
  const onSubmit: SubmitHandler<BookFormInputs> = async (data) => {
    ...
    try {
      ...
      // Here you could add a toast notification saying "Failed to save"
    } finally {
      ...
      <div className="max-w-[1000px] mx-auto">
        ...
        <nav className="mb-4">
          <p className="text-[#4B5563] text-xs font-medium tracking-wide">
            Dashboard / Books / <span className="text-[#DC2626]">Add New Book</span>
          </p>
        </nav>
        ...
        <div className="flex items-center justify-between mb-10">
          <div>
            <h2 className="text-white text-3xl font-bold font-headline tracking-tight">Add New Book</h2>
            <p className="text-[#6B7280] mt-1">Fill in the details to add a book to the catalogue</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="col-span-1 md:col-span-12 lg:col-span-5 space-y-6">
            ...
            <div className="bg-[#111111] border border-[#1F1F1F] rounded-2xl p-6 shadow-sm">
              ...
              <div className="w-full h-52 border-2 border-dashed border-[#2A2A2A] bg-[#0D0D0D] rounded-xl flex flex-col items-center justify-center group cursor-pointer hover:border-[#DC2626]/40 transition-all">
                <div className="w-12 h-12 rounded-full bg-[#1A1A1A] flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <CloudUpload className="text-[#DC2626] w-6 h-6" />
                </div>
                <p className="text-white font-semibold text-sm">Upload High-Res Cover</p>
                <p className="text-[#6B7280] text-xs mt-1">Drag and drop or click to upload</p>
                <p className="text-[10px] text-[#4B5563] mt-3 uppercase tracking-widest font-bold">PNG, JPG up to 10MB</p>
              </div>
            </div>
            ...
            <div className="bg-[#111111] border border-[#1F1F1F] rounded-2xl p-6 shadow-sm overflow-hidden relative">
              ...
              <div className="bg-[#0D0D0D] border border-[#1F1F1F] rounded-xl p-4 flex gap-4 hover:shadow-2xl hover:shadow-black transition-all">
                <div className="w-24 h-32 rounded-lg bg-[#1a1a1a] flex-shrink-0 flex items-center justify-center overflow-hidden border border-[#1F1F1F]">
                  <ImageIcon className="text-[#2A2A2A] w-8 h-8 opacity-40" />
                </div>
                <div className="flex flex-col justify-between py-1">
                  <div>
                    <p className="text-[#6B7280] text-[10px] font-black uppercase tracking-widest mb-1">New Entry</p>
